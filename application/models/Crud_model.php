...
class Crud_model extends CI_Model
{
	...
	*/
	 ...
	 * @author Alfonso
	 *
	 * Description: Consulta información de una materia.
	*/
	public function getInfoAsigEE($nrc)
	{
		$this->db->select('maestro.nombMtro, maestro.numMtro');
		$this->db->where('nrc', $nrc);
		$this->db->from('carga');
		$this->db->join('maestro', 'carga.numMtro = maestro.numMtro');
		$query = $this->db->get();
		return $query->result();
	}
